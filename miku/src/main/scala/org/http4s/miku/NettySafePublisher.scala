...
private[http4s] final class NettySafePublisher(pub: Publisher[HttpContent]) extends Publisher[Chunk[Byte]]{
...
object NettySafePublisher {
  private[http4s] class ChunkEmitSubscriber(subscriber: Subscriber[_ >: Chunk[Byte]]) extends Subscriber[HttpContent]{
